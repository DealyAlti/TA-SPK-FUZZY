...
<div class="wrap">
    ...
    @if(!empty($warnings) && count($warnings))
        <div class="section">
            <div class="head"><h3><i class="fa fa-exclamation-triangle"></i> Peringatan</h3></div>
            <div class="body">
                @foreach($warnings as $w)
                    <div style="border:1px solid #fecaca;background:#fff7ed;border-radius:14px;padding:12px 14px;margin-bottom:10px;">
                        <b>{{ $w['title'] }}</b>
                        <div style="color:#6b7280;font-size:13px;margin-top:4px;">{{ $w['text'] }}</div>
                        <a href="{{ $w['url'] }}" class="btn-solid" style="margin-top:10px;padding:8px 12px!important;">
                            <i class="fa fa-arrow-right"></i> Buka
                        </a>
                    </div>
                @endforeach
            </div>
